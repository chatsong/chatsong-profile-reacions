...
function init(){
  ...
  w.innerHTML = '<div style="font-weight:800;font-size:15px;color:#fff;margin-bottom:12px;">🏆 Artist Endorsements</div><div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;"><div style="text-align:center;min-width:50px;"><div style="font-size:28px;font-weight:900;color:#FFD700;" id="ca-'+user+'">—</div><div style="font-size:10px;color:#888;" id="cn-'+user+'">loading</div></div><div style="flex:1;display:flex;gap:2px;font-size:22px;" id="st-'+user+'"></div></div><div style="font-size:11px;color:#e0e0e0;margin-bottom:12px;">Community Badges:</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px;" id="eb-'+user+'"></div><div style="font-size:11px;color:#e0e0e0;margin-bottom:8px;">Fan Shoutouts:</div><div id="sg-'+user+'" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:6px;max-height:180px;overflow-y:auto;"></div><div style="display:flex;gap:6px;margin-top:10px;"><input id="si-'+user+'" maxlength="60" placeholder="Drop a shout..." style="flex:1;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;padding:8px 10px;color:#fff;font-size:12px;"><button id="sb-'+user+'" style="background:#1DB954;border:none;border-radius:8px;padding:8px 12px;color:#fff;font-weight:700;font-size:12px;cursor:pointer;">Post</button></div>';
